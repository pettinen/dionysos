...
def greed(game):
    ...
    for from_user_id in user_ids:
        ...
        cards = redis.hgetall(game.redis_key(other_cards))
...
def magic(game):
    card = Card("magic")
    deck = redis.lrange(game.redis_key("deck"), 0, -1)
    random.shuffle(deck)
    deck.append(card.id)
    redis.delete(game.redis_key("deck"))
    redis.rpush(game.redis_key("deck"), *deck)
    game.emit(
        "card-effect",
        {
...
def too_many_cards(game):
    redis.ltrim(game.redis_key("deck"), 2, -1)
    if redis.llen(game.redis_key("deck")) == 0:
        game.end()
...
def generosity(game):
    ...
    if redis.hexists(game.redis_key(f"user:{g.user.id}:active-cards"), card.id):
        use_cards = [
            int(card_id)
            for card_id in redis.lrange(
                game.redis_key(f"user:{g.user.id}:use-cards"), 0, -1
            )
        ]
...
#        if redis.hexists(key, card.id):
#            next_card = redis.lindex(game.redis_key('deck'), 0)
