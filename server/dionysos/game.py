...
from .utils import apos, base57_random
...
class Card:
    def __init__(self, card_id):
        ...
        cur.close()
        self.type = {"id": type_id, "name": type_name}
...
class Game:
    ...
    def create(cls, name, max_players, remote, password=None):
        ...
        name = name.strip()
        if len(name) < app.config["GAME_NAME_LENGTH"]["min"]:
            raise ValueError("game-name-too-short")
        if len(name) > app.config["GAME_NAME_LENGTH"]["max"]:
            raise ValueError("game-name-too-long")
        elif not name:
            name = f"{g.user.name}{apos}s game"

        if (
            max_players < app.config["MAX_PLAYERS_LIMITS"]["min"]
            or max_players > app.config["MAX_PLAYERS_LIMITS"]["max"]
        ):
        ...
        unique_id = False
        while not unique_id:
            id_ = base57_random(app.config["GAME_ID_LENGTH"])
    ...
    def add_active_card(self, card, user):
        ...
        if user == "all":
            self.emit(
                "active-card-added",
                {"cardID": card.id, "userID": "all", "counter": self.counter},
            )
        elif card.visibility == "all":
            self.emit(
                "active-card-added",
                {"cardID": card.id, "userID": user.id, "counter": self.counter},
            )
        else:
        ...
        redis_rv = redis.hset(key, card.id, duration)
        if redis_rv != 1:
            logging.error(
                f"Game.add_active_card added {redis_rv} fields to DB; expected 1"
            )
    ...
    def add_player(self, user):
        ...
        self.counter += 1
        socketio.emit("game-updated", {"id": self.id, "playerCount": self.player_count})
        self.emit(
            "game-joined", {"id": user.id, "name": user.name, "counter": self.counter}
        )
        join_room(self.room)
        ...
        def check_durations(user_id):
            ...
            for card_id, duration in cards.items():
                ...
                if duration == 1:
                    ...
                    redis.hincrby(key, card.id, -1)

        check_durations("all")
        check_durations(self.current_player)

        def current_player_skips():
            turn_skips = redis.hget(self.redis_key("turn-skips"), self.current_player)
    ...
    def current_player(self):
        user_id = redis.get(self.redis_key("current-player"))
    ...
    def current_player(self, new_player_id):
        redis.set(self.redis_key("current-player"), new_player_id)

    def delete(self):
        """Do not use the Game object after calling this."""
        keys = redis.scan_iter(self.redis_key("*"))
        if keys:
            redis.delete(*keys)
        cur = db.cursor()
    ...
    def discard(self, card, user):
        redis.rpush(self.redis_key("discard"), card.id)
        self.counter += 1
        self.emit(
            "card-discarded",
            {"cardID": card.id, "userID": user.id, "counter": self.counter},
    ...
    def draw_card(self):
        card_id = redis.lpop(self.redis_key("deck"))
        if card_id is None:
            # TODO: log this
        ...
        card = Card(int(card_id))
        if card.type["id"] == "use":
            redis.rpush(self.redis_key(f"user:{g.user.id}:use-cards"), card.id)
            ...
            if card.visibility == "all":
                self.emit("public-card-drawn", {"cardID": card.id, "userID": g.user.id})
            elif card.visibility == "player":
                self.emit("private-card-drawn", {"userID": g.user.id})

        if redis.llen(self.redis_key("deck")) == 0:
            self.end()
        ...
        if card.type["id"] != "use" and card.text_id in card_handlers:
    ...
    def end(self):
        ...
        cur = db.cursor()
        cur.execute(
            "UPDATE games SET started = false, ended = true WHERE id = %s;", [self.id]
        )
        if cur.rowcount != 1:
    ...
    def has_player(self, user_id):
        cur = db.cursor()
        cur.execute(
            "SELECT users_games.user_id FROM users_games"
            " WHERE users_games.game_id = %s;",
            [self.id],
        )
    ...
    def next_player(self):
        ...
        player_order = redis.lrange(self.redis_key("player-order"), 0, -1)
    ...
    def players(self):
        ...
        players = [{"id": user_id, "name": username} for user_id, username in cur]
        cur.close()
        return players
    ...
    def remove_active_card(self, card_id, user_id):
        if user_id == "all":
            ...
            key = self.redis_key(f"user:{user_id}:active-cards")
        if redis.hdel(key, card_id) != 1:
            pass  # TODO: log this
        socketio.emit("active-card-removed", {"cardID": card_id, "userID": user_id})

    def remove_use_card(self, card_id, user):
        key = self.redis_key(f"user:{user.id}:use-cards")
        if redis.lrem(key, 1, card_id) != 1:
            pass  # TODO: log this
    ...
    def remove_player(self, user):
        ...
        redis.lrem(self.redis_key("player-order"), 0, user.id)
        redis.hdel(self.redis_key("turn-skips"), user.id)
        keys = redis.keys(self.redis_key(f"{user.id}:*"))
        if keys:
            redis.delete(*keys)
        ...
        if self.player_count == 0:
            ...
            new_order = [
                int(player_id)
                for player_id in redis.lrange(self.redis_key("player-order"), 0, -1)
            ]
            self.emit("player-order-changed", new_order)
            socketio.emit(
                "game-updated", {"id": self.id, "playerCount": self.player_count}
            )
    ...
    def skip_turns(self, count, user_id):
        redis.hincrby(self.redis_key("turn-skips"), user_id, count)
    ...
    def start(self):
        ...
        redis.rpush(self.redis_key("player-order"), *player_ids)
        ...
        redis.rpush(self.redis_key("deck"), *card_ids)

        first_player = int(redis.lindex(self.redis_key("player-order"), 0))
    ...
    def use_card(self, card, user):
        key = self.redis_key(f"user:{user.id}:use-cards")
        if str(card.id) not in redis.lrange(key, 0, -1):
        ...
        self.emit("use-card-used", {"cardID": card.id, "userID": user.id})
